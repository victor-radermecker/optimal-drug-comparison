...
for name, df in formularies_raw.items():
    ...
    if name == "OptimalPAP":
        ...
        # Add explicit Humira and biosimilar coverage for Optimal
        # This ensures Cyltezo and other biosimilars are covered
        formularies[name] = optimal_combined

        # Print diagnostic information about the Optimal formulary
        print(f"Optimal formulary size: {len(formularies[name])}")
        print(
            f"Humira or biosimilars in Optimal: {formularies[name]['ndcdescription'].str.contains('Humira|Cyltezo', case=False).sum()}"
        )
...
def is_biosimilar_covered(drug_name, formulary_drugs):
    """
    Check if a drug (Humira or any of its biosimilars) is covered by a formulary.
    If drug_name contains any string in humira_biosimilars, then coverage is
    True if the formulary_drugs contain any of the strings in humira_biosimilars.
    """
    drug_lower = drug_name.lower()

    # 1) Check if the claim's drug is either Humira or a known Humira biosimilar
    if any(biosim.lower() in drug_lower for biosim in humira_biosimilars):
        # 2) If yes, then consider it covered if *any* biosimilar/Humira string
        #    also appears in the formulary_drugs list
        return any(
            any(biosim.lower() in fdrug.lower() for biosim in humira_biosimilars)
            for fdrug in formulary_drugs
        )

    # If it's not recognized as Humira or a known biosimilar, return False
    return False
